...
import com.c4soft.quiz.web.SkillTestController.InvalidQuizException;
...
public class ExceptionHandlers {
	...
	@ApiResponse(responseCode = "422", content = { @Content(schema = @Schema(implementation = ValidationProblemDetail.class)) })
	public ResponseEntity<ValidationProblemDetail> handleMethodArgumentNotValid(MethodArgumentNotValidException ex) {
		final var detail = new ValidationProblemDetail(
				ex.getMessage(),
				ex.getFieldErrors().stream().collect(Collectors.toMap(FieldError::getField, FieldError::getCode)));
		return ResponseEntity.status(detail.getStatus()).body(detail);
	}

	@ExceptionHandler(ConstraintViolationException.class)
	@ApiResponse(responseCode = "422", content = { @Content(schema = @Schema(implementation = ValidationProblemDetail.class)) })
	public ResponseEntity<ValidationProblemDetail> handleConstraintViolation(ConstraintViolationException ex) {
		final var problem = new ValidationProblemDetail(
				ex.getMessage(),
				ex.getConstraintViolations().stream().collect(Collectors.toMap(cv -> cv.getPropertyPath().toString(), ConstraintViolation::getMessage)));
	...
	public ResponseEntity<ProblemDetail> handleInvalidQuiz(InvalidQuizException ex) {
		final var problem = ProblemDetail.forStatusAndDetail(HttpStatus.CONFLICT, ex.getMessage());
		return ResponseEntity.status(problem.getStatus()).body(problem);
	}

	@ExceptionHandler(QuizAlreadyHasAnAnswerException.class)
	@ApiResponse(responseCode = "409", content = { @Content(schema = @Schema(implementation = ProblemDetail.class)) })
	public ResponseEntity<ProblemDetail> handleQuizAlreadyHasAnAnswer(QuizAlreadyHasAnAnswerException ex) {
		final var problem = ProblemDetail.forStatusAndDetail(HttpStatus.CONFLICT, ex.getMessage());
		return ResponseEntity.status(problem.getStatus()).body(problem);
	}
