...
public class QuizAuthentication extends OAuthentication<OpenidClaimSet> {
	...
	public static final String SPEL_IS_MODERATOR_OR_TRAINER = "hasAnyAuthority('moderator', 'trainer')";
	...
	public QuizAuthentication(OpenidClaimSet claims, Collection<? extends GrantedAuthority> authorities, String tokenString) {
		super(claims, authorities, tokenString);
		final var authoritiesStrings = authorities.stream().map(GrantedAuthority::getAuthority).collect(Collectors.toSet());
