...
public interface QuizRepository extends JpaRepository<Quiz, Long>, JpaSpecificationExecutor<Quiz> {
  ...
  static Specification<Quiz> authoredBy(String authorName) {
    return (root, query, cb) -> cb.like(cb.upper(root.get(Quiz_.authorName)),
        "%%%s%%".formatted(authorName.toUpperCase()));
  }

  static Specification<Quiz> titleContains(String title) {
    return (root, query, cb) -> cb.like(cb.upper(root.get(Quiz_.title)),
        "%%%s%%".formatted(title.toUpperCase()));
  }
