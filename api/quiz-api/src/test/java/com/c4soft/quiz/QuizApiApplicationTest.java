...
@SpringBootTest(properties = {"hostname=localhost"})
@ActiveProfiles("h2")
...
class QuizApiApplicationTest {
  ...
  void givenUserIsCh4mp_whenGoingThroughQuizNominalCrudOperations_thenOk()
    ...
    assertEquals("OAuth2 and OpenID in web echosystem with Spring",
        ((JSONObject) actual.get(0)).get("title"));
    ...
    // Update draft title
    api.put(new QuizUpdateDto("Updated title", true, true, true, false), "/quizzes/{quiz-id}",
        draftId).andExpect(status().isAccepted());
    ...
    assertEquals(2L, actual.size());
    assertEquals(1L,
        actual.stream().filter(q -> (boolean) ((JSONObject) q).get("isPublished")).toList().size());

    // Create a new quiz
    final var quiz1Id = api
        .post(new QuizUpdateDto("Second Quiz", true, true, true, false), "/quizzes")
        .andExpect(status().isCreated()).andReturn().getResponse().getHeader(HttpHeaders.LOCATION);
    api.get("/quizzes?titleLike=second").andExpect(status().isOk())
        .andExpect(jsonPath("$.*.title", hasItems("Second Quiz")));
    ...
    api.put(new QuestionUpdateDto("What is the answer to machin?", "", "The answer is truc."),
        "/quizzes/{quiz-id}/questions/{question-id}", quiz1Id, question10Id)
        .andExpect(status().isAccepted());
    api.get("/quizzes/{quiz-id}", quiz1Id).andExpect(status().isOk())
    .andExpect(jsonPath("$.questions[1].label", is("What is the answer to machin?")))
    .andExpect(jsonPath("$.questions[1].priority", is(1)))
        .andExpect(jsonPath("$.questions[0].label", is("bidule")))
        .andExpect(jsonPath("$.questions[0].priority", is(0)));

    // Add 2 choices
    final var choice100Id = api
        .post(new ChoiceUpdateDto("truc", true),
            "/quizzes/{quiz-id}/questions/{question-id}/choices", quiz1Id, question10Id)
        .andExpect(status().isCreated()).andReturn().getResponse().getHeader(HttpHeaders.LOCATION);
    final var choice101Id = api
        .post(new ChoiceUpdateDto("chouette", false),
            "/quizzes/{quiz-id}/questions/{question-id}/choices", quiz1Id, question10Id)
        .andExpect(status().isCreated()).andReturn().getResponse().getHeader(HttpHeaders.LOCATION);
    api.get("/quizzes/{quiz-id}", quiz1Id).andExpect(status().isOk())
        .andExpect(jsonPath("$.questions[1].choices.*.label", hasItems("truc", "chouette")));
    api.put(new ChoiceUpdateDto("chose", true),
        "/quizzes/{quiz-id}/questions/{question-id}/choices/{choice-id}", quiz1Id, question10Id,
        choice101Id).andExpect(status().isAccepted());
    api.get("/quizzes/{quiz-id}", quiz1Id).andExpect(status().isOk())
    ...
    // Delete a question
    api.delete("/quizzes/{quiz-id}/questions/{question-id}/choices/{choice-id}", quiz1Id,
        question10Id, choice100Id).andExpect(status().isAccepted());
    api.get("/quizzes/{quiz-id}", quiz1Id).andExpect(status().isOk())
    ...
    api.get("/quizzes/{quiz-id}", draftId).andExpect(status().isOk())
  ...
  void givenUserIsATrainee_whenGoingThroughSkillTestNominalOperations_thenOk()
      throws UnsupportedEncodingException, Exception {
    var quiz = (JSONObject) parse(api.get("/quizzes").andExpect(status().isOk()).andReturn(),
        JSONArray.class).get(0);
    ...
    final var tontonPirate = new UserRepresentation();
    tontonPirate.setEmail("tonton-pirate@c4-soft.com");
    when(keycloakAdminApi.adminRealmsRealmUsersGet("quiz", Optional.empty(), Optional.empty(),
        Optional.empty(), Optional.empty(), Optional.of(true), Optional.empty(), Optional.empty(),
        Optional.empty(), Optional.empty(), Optional.empty(), Optional.empty(), Optional.empty(),
        Optional.empty(), Optional.of("tonton-pirate")))
            .thenReturn(ResponseEntity.ok(List.of(tontonPirate)));
    api.perform(get("/skill-tests/{quizId}/{traineeName}", quizId.toString(), "tonton-pirate"))
        .andExpect(status().isOk()).andExpect(jsonPath("$.score", is(-50.0)));

    api.put(perfectAnswer, "/skill-tests").andExpect(status().isAccepted())
        .andExpect(jsonPath("$.score", is(100.0)));
    api.perform(get("/skill-tests/{quizId}/{traineeName}", quizId.toString(), "tonton-pirate"))
        .andExpect(status().isOk()).andExpect(jsonPath("$.score", is(100.0)));
  }

  private <T> T parse(MvcResult result, Class<T> clazz)
      throws UnsupportedEncodingException, ParseException {
    return new JSONParser(JSONParser.MODE_PERMISSIVE)
        .parse(result.getResponse().getContentAsString(), clazz);
  }
  ...
  public static class Fixtures {
    ...
    public static Quiz openIdTraingQuiz() {
      final var quiz = new Quiz("OAuth2 and OpenID in web echosystem with Spring", trainer1,
          new Question("Question 1", "", 0, "Good is right", new Choice("good", true),
              new Choice("bad", false)),
          new Question("Question 2", "", 1, "Bad is wrong", new Choice("bad", false),
              new Choice("good", true)));
